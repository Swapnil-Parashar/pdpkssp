...
export default {
    ...
    computed: {
        ...
        images: function () {
            return this.$store.getters["frontendProduct/showImages"].map(
                (image) => image.replace("/conversions/", "/")
            );
        },
        videos: function () {
            ...
            if (typeof this.$route.params.slug !== "undefined") {
                ...
                this.$store
                    ...
                    .then((res) => {
                        ...
                        this.$store.commit('updateProductImages', res.data.data.images.map(image => image.replace('/conversions/', '/')));
